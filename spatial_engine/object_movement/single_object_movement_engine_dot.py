...
QUESTION_TEMPLATES = {
    "tapvid3d_total_distance": [
        "How far did the annotated point in Image-1 travel between the two shots?",
        "What is the total distance the annotated point in Image-1 moved from its first position?",
        "Could you give me the magnitude of the annotated point's displacement in Image-1?",
        "I'm curious about the overall traveled distance of the annotated point in Image-1 in millimeters?",
        "Please specify the total movement distance of the annotated point in Image-1.",
        "From start to end, how many units (mm) did the annotated point in Image-1 shift in space?",
        "What is the absolute distance covered by the annotated point in Image-1 between these images?",
        "Please provide the length of the path the annotated point in Image-1 took.",
        "Does the annotated point in Image-1 have a large or small travel distance, and how much is it?",
        "Could you measure the total displacement for the annotated point in Image-1's movement?",
        "Could you calculate the overall distance from the annotated point in Image-1's first pose to the second?",
        "In numerical terms, how far did the annotated point in Image-1 go?",
        "Is there a measurement for the annotated point in Image-1's complete travel distance in mm?",
        "Between these two viewpoints, what's the movement distance of the annotated point in Image-1?",
        "I want to know the total length of the annotated point in Image-1's translation.",
        "Please share how many millimeters the annotated point in Image-1 has moved in total.",
        "Is the annotated point in Image-1's movement more than a few millimeters, and how many exactly?",
        "How do you quantify the complete distance traveled by the annotated point in Image-1?",
        "What is the final measure of the distance the annotated point in Image-1 traversed?",
        "How large is the gap between the annotated point in Image-1's initial and final positions in mm?",
        "What's the annotated point in Image-1's net travel distance from the first shot to the second shot?",
        "I'm interested in the point-to-point distance of the annotated point in Image-1's shift.",
        "Could you give an estimate or exact number of how far the annotated point in Image-1 has moved?",
        "Does the data say how many millimeters separate the annotated point in Image-1's old and new positions?",
        "What's the measurement of the annotated point in Image-1's movement vector's length?",
        "Check the final distance: how many mm did the annotated point in Image-1 shift overall?",
        "In your analysis, what's the total movement distance for the annotated point in Image-1?",
        "If we consider the start and end points, how far apart are they for the annotated point in Image-1?",
        "Please clarify the total displacement distance in millimeters for the annotated point in Image-1.",
        "Give me the final figure for the annotated point in Image-1's traveled distance in mm?"
    ],
    "tapvid3d_displacement_vector": [
        "The first image is oriented such that positive X is right, Y is down, and Z is forward. What is the overall displacement vector of the annotated point in Image-1?",
        "The first image is oriented such that positive X is right, Y is down, and Z is forward. Could you provide the annotated point in Image-1's movement vector in 3D coordinates?",
        "The first image is oriented with X->right, Y->down, Z->forward. Please specify the `[ x , y , z ]` displacement vector of the annotated point in Image-1 in mm?",
        "The first image sets X=right, Y=down, Z=forward as positive. I'm curious about the entire translation vector from the first to second position for the annotated point in Image-1.",
        "Given X->right, Y->down, Z->forward, what does the annotated point in Image-1's shift vector look like in millimeters for each axis?",
        "We define X as right, Y as down, Z as forward. How do you represent the 3D displacement of the annotated point in Image-1 as a vector?",
        "Considering the first image orientation (X=right, Y=down, Z=forward), could you express the annotated point in Image-1's movement as a coordinate vector in mm?",
        "The first image uses positive X=right, Y=down, Z=forward. From the start to the end, what's the `[ x , y , z ]` translation of the annotated point in Image-1?",
        "With X=right, Y=down, Z=forward, I want the annotated point in Image-1's movement in vector form, specifying the offsets in each axis.",
        "Remember X->right, Y->down, Z->forward. Please give me the exact displacement as `[ x , y , z ]` in mm for the annotated point in Image-1.",
        "The coordinate system is X=right, Y=down, Z=forward. Do we have the final 3D translation vector for the annotated point in Image-1's shift?",
        "We assume positive X=right, Y=down, Z=forward. How can we break down the annotated point in Image-1's movement into x, y, z components in mm?",
        "The first image orientation sets X, Y, Z as right, down, forward. Which vector best describes the annotated point in Image-1's overall translation?",
        "Using X=right, Y=down, Z=forward, could you detail the annotated point in Image-1's movement vector so I see how it moved along each axis?",
        "Positive X is right, Y is down, Z is forward. Is there a coordinate triple representing the annotated point in Image-1's new position minus the old?",
        "If we consider X=right, Y=down, Z=forward, tell me the displacement vector that sums up how the annotated point in Image-1 traveled in `[ x , y , z ]` mm.",
        "The coordinate definition is X->right, Y->down, Z->forward. What's the resulting 3D vector for the annotated point in Image-1's net translation?",
        "In a system where X=right, Y=down, Z=forward, I'd like the translation vector: `[ x_movement , y_movement , z_movement ]` in mm for the annotated point in Image-1.",
        "Given the orientation X=right, Y=down, Z=forward, does the data give us the annotated point in Image-1's movement as a standard `[ x , y , z ]` vector?",
        "From the start to end, with X=right, Y=down, Z=forward, what's the `[ x , y , z ]` vector describing the annotated point in Image-1's translation?",
        "Under the assumption X=right, Y=down, Z=forward, I want the final movement as a three-dimensional vector describing the annotated point in Image-1's translation.",
        "Taking X=right, Y=down, Z=forward as the scene basis, how do we express the total movement as `[ x , y , z ]` in mm for the annotated point in Image-1?",
        "If the first image orientation is X=right, Y=down, Z=forward, could you share the numeric triple for the annotated point in Image-1's displacement along `[ x , y , z ]`?",
        "We define X=right, Y=down, Z=forward. I'm looking for the final movement as a `[ x , y , z ]` vector for the annotated point in Image-1.",
        "Between these images, using X=right, Y=down, Z=forward, what's the vector that the annotated point in Image-1 traveled in mm?",
        "The coordinate system is X=right, Y=down, Z=forward. Is there a 3D vector capturing the net offset from the original spot for the annotated point in Image-1?",
        "In the orientation where X=right, Y=down, Z=forward, how do we write the annotated point in Image-1's translation in the form `[ x_offset , y_offset , z_offset ]` mm?",
        "Assuming the first image sets X=right, Y=down, Z=forward, check the final difference in each axis-this forms a vector for the annotated point in Image-1's movement, correct?",
        "Given X=right, Y=down, Z=forward, I'd like you to list the annotated point in Image-1's total displacement vector `[ x , y , z ]` in mm.",
        "The first image orients X->right, Y->down, Z->forward. Please specify that vector showing exactly how the annotated point in Image-1 moved in all three dimensions."
    ]
}
...
class TwoFrameVideoQAEngineDot:
    def __init__(self, question_type, sub_dataset):
        ...
        self.object_not_moving_threshold = 0.1 if self.sub_dataset == "drivetrack" else 0.01
        self.camera_not_moving_threshold = 0.1 if self.sub_dataset == "drivetrack" else 0.01
        self.future_frame_windows = 5 if self.sub_dataset == "drivetrack" else 1e8 # for dirvetrack, due to inaccurate of extrinsics, we only consider image pairs within 5 frames.
    ...
    def annotate_image(self, image, point):
        ...
        # Draw circle
        cv2.circle(annotated_img, (x, y), self.image_width // 100, color, -1)
        
        return annotated_img

    def format_training_samples(self, sample_pairs, intrinsics, scene_id, points_pos_world, points_pos_cam, image_height, image_width, extrinsics_w2c, base_img_dir, img_output_dir):
        """
        ...
        for sample_pair in sample_pairs:
            ...
            # read the images and annotated it
            img_path1 = os.path.join(base_img_dir, scene_id, f"{frame1:05d}.jpg")
            img_path2 = os.path.join(base_img_dir, scene_id, f"{frame2:05d}.jpg")
            save_dir = os.path.join(img_output_dir, scene_id)
            mmengine.mkdir_or_exist(save_dir)
            save_path1 = os.path.join(save_dir, f"{frame1:05d}_{point_index}_annotated.jpg")
            save_path2 = os.path.join(save_dir, f"{frame2:05d}.jpg")

            # for image1, if not exists, annotated the images and save
            if not os.path.exists(save_path1):
                img1 = cv2.imread(img_path1)
                annotated_img1 = self.annotate_image(img1, (int(point_2d_normalized_1[0] * image_width), int(point_2d_normalized_1[1] * image_height)))
                cv2.imwrite(save_path1, annotated_img1)
            
            # for image2, if not exists, directly copy because image2 is not annotated
            if not os.path.exists(save_path2):
                shutil.copy(img_path2, save_path2)

            images = [f"{scene_id}/{frame1:05d}_{point_index}_annotated.jpg", f"{scene_id}/{frame2:05d}.jpg"]

            entry = {
                "id": f"{scene_id}_{frame1}_{frame2}_{point_index}_ann",
                "image": images,
                "conversations": conversation,
                "height_list": [image_height] * len(images),
                "width_list": [image_width] * len(images),
                "gt_value": int(displacement_distance * 1000) if self.question_type == "total_distance" else displacement_vector_cam1.tolist(), # note vector stored in meters
    ...
    def generate_qa_training_single_scene(self, input_file, base_img_dir, npoints_per_group=5, npairs_per_bin=1e8, img_output_dir="", augment=True, augment_ratio=1.0):
        """
        split_name should be: [pstudio | drivetrack | adt]
        """
        ...
        scene_img_dir = os.path.join(base_img_dir, scene_id)
    ...
    def generate_qa_training_data(self, scene_id_list, source_data_root, base_img_dir, output_dir, output_file, img_output_dir, npoints_per_group, npairs_per_bin, augment, augment_ratio=1.0, max_samples=-1, num_workers=20):
        scene_files = [os.path.join(source_data_root, f"{scene_id}.npz") for scene_id in scene_id_list]

        tasks = [(scene_file, base_img_dir, npoints_per_group, npairs_per_bin, img_output_dir, augment, augment_ratio) for scene_file in scene_files]
    ...
    def generate_qa_eval_data(self, scene_id_list, source_data_root, base_img_dir, output_dir, output_file, img_output_dir, npoints_per_group, npairs_per_bin, augment, augment_ratio=0.3, max_samples=300, num_workers=20):
        ...
        # save to file
        with open(output_file.replace(".jsonl", "_orig.jsonl"), 'w') as f:
            for entry in eval_data:
                f.write(json.dumps(entry) + '\n')
        
        # subsample the data
        if max_samples > 0 and len(eval_data) > max_samples:
            subsampled_eval_data = random.sample(eval_data, max_samples)
        
        # save to file
        with open(output_file, 'w') as f:
            for entry in subsampled_eval_data:
                f.write(json.dumps(entry) + '\n')

        # print statics of original data
        point_not_moving_count = sum(1 for entry in train_data if entry["point_moving"] == 0)
        point_moving_count = len(train_data) - point_not_moving_count
        cam_not_moving_count = sum(1 for entry in train_data if entry["cam_moving"] == 0)
        cam_moving_count = len(train_data) - cam_not_moving_count
        ...
        # 打印信息
        print(f"Original evaluation data saved to {output_file.replace('.jsonl', '_orig.jsonl')}. In total, there are {len(eval_data)} samples.")   
        print(f"Object not moving: {point_not_moving_count}, Object moving: {point_moving_count}")
        print(f"Camera not moving: {cam_not_moving_count}, Camera moving: {cam_moving_count}")

        # print statics of subsampled data
        point_not_moving_count = sum(1 for entry in subsampled_eval_data if entry["point_moving"] == 0)
        point_moving_count = len(subsampled_eval_data) - point_not_moving_count
        cam_not_moving_count = sum(1 for entry in subsampled_eval_data if entry["cam_moving"] == 0)
        cam_moving_count = len(subsampled_eval_data) - cam_not_moving_count

        print(f"Subsampled evaluation data saved to {output_file}. In total, there are {len(subsampled_eval_data)} samples.")   
        print(f"Object not moving: {point_not_moving_count}, Object moving: {point_moving_count}")
        print(f"Camera not moving: {cam_not_moving_count}, Camera moving: {cam_moving_count}")
...
if __name__ == "__main__":
    ...
    sub_datasets = ["adt", "pstudio"] # currently, we do not use drivetrack because we still cannot deal with the errors of extrinsics.
    ...
    train_output_dir = f"training_data_v2/object_movement_dot/{version}"
    val_output_dir = f"evaluation_data_v2/object_movement_dot/{version}"
    ...
    base_img_dir = "data/my_tapvid3d_images"
    ...
    for q_type in train_question_samples.keys():
        for sub_dataset in sub_datasets:
            ...
            img_output_dir = f"{val_output_dir}/{sub_dataset}/{q_type}_images"
            mmengine.mkdir_or_exist(output_dir)
            # remove img_output_dir if it exists
            if os.path.exists(img_output_dir):
                shutil.rmtree(img_output_dir)
            mmengine.mkdir_or_exist(img_output_dir)

            train_output_file = os.path.join(output_dir, f"{sub_dataset}_{q_type}_val.jsonl")

            qa_engine = TwoFrameVideoQAEngineDot(question_type=q_type, sub_dataset=sub_dataset)
            qa_engine.generate_qa_eval_data(scene_id_list=scene_id_list, source_data_root=source_data_root, base_img_dir=base_img_dir, output_dir=output_dir, output_file=train_output_file,
                                                 img_output_dir=img_output_dir, npoints_per_group=1, npairs_per_bin=1,
                                                 augment=False, max_samples=300, num_workers=20)

    # TODO: maybe we need to do rigid body segmentation for each sub-dataset first 
    # generate train data
    for q_type in train_question_samples.keys():
        for sub_dataset in sub_datasets:
            ...
            img_output_dir = f"{train_output_dir}/{sub_dataset}/{q_type}_images_{npoints_per_group}points_{npairs_per_bin}pairs"
            ...
            qa_engine = TwoFrameVideoQAEngineDot(question_type=q_type, sub_dataset=sub_dataset)
            qa_engine.generate_qa_training_data(scene_id_list=scene_id_list, source_data_root=source_data_root, base_img_dir=base_img_dir, output_dir=output_dir, output_file=train_output_file,
